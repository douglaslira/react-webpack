...
class Auth extends React.Component {
	...
	handleSubmit(e) {
		...
		AuthService.login(username, password).then((response)=>{
			...
			this.props.authAction(response);
			...
			<div className="col-md-6 offset-md-3">
				...
				<form name="form" onSubmit={this.handleSubmit}>
					...
					<div className={'form-group' + (submitted && !password ? ' has-error' : '')}>
						...
						<input type="password" className="form-control" autoComplete="false" name="password" value={password} onChange={this.handleChange} />
						{submitted && !password && <div className="help-block">Password is required</div>}
					</div>
					<div className="form-group">
						<button className="btn btn-primary" disabled={loading}>Access</button>
...
const AuthComponent = connect(null, mapDispatchToProps)(Auth);
